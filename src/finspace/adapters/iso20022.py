...
class ISO20022PaymentBuilder:
    ...
    @staticmethod
    def _sub(
        etree: Any, parent: Any, namespace: str, name: str, text: str | None = None, **attrs: str
    ) -> Any:
    ...
    def _pain(self, record: Mapping[str, Any]) -> bytes:
        ...
        self._sub(
            ...
            ns,
            "EndToEndId",
            str(record.get("end_to_end_id", "FINSPACE-E2E")),
        )
    ...
    def _pacs(self, record: Mapping[str, Any]) -> bytes:
        ...
        amount = Decimal(str(record["amount"]))
        self._sub(
            etree, transaction, ns, "IntrBkSttlmAmt", f"{amount:.2f}", Ccy=str(record["currency"])
        )
