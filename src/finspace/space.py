...
class Space:
        ...
        for field_spec in self.schema.fields:
            ...
            if not active:
                if field_spec.name in record:
                    raise RecordValidationError(
                        f"field {field_spec.name!r} is not active for this record"
                    )
                continue
            ...
            index = next(
                (i for i, candidate in enumerate(values) if _canonical(candidate) == key), None
            )
            if index is None:
                raise RecordValidationError(
                    f"value {value!r} is not allowed for field {field_spec.name!r} in this context"
                )
            if field_spec.name in self.fixed and _canonical(self.fixed[field_spec.name]) != key:
                raise RecordValidationError(
                    f"field {field_spec.name!r} must equal conditioned value {self.fixed[field_spec.name]!r}"
                )
    ...
    def _record_from_tokens(self, tokens: Sequence[str | int]) -> dict[str, JSONValue]:
        ...
        for field_spec in self.schema.fields:
            ...
            try:
                ...
                raise RuntimeError(
                    f"compiled engine returned invalid token {tokens[cursor]!r} for {field_spec.name!r}"
                ) from error
            context[field_spec.name] = value
            cursor += 1
        if cursor != len(tokens):
    ...
    def sample(
        ...
        *,
        ...
        ranks = (
            [rng.randrange(self.count) for _ in range(n)]
            if replace
            else rng.sample(range(self.count), n)
        )
        if with_ranks:
    ...
    def sample_stratified(
        ...
        for value, allocation in zip(values, allocations, strict=False):
