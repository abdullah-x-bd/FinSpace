...
def command_verify_environment(args: argparse.Namespace) -> int:
    ...
    status = (
        "matched" if execution["environment_hash"] == digest(manifest) else "environment-mismatch"
    )
...
def command_replay_execution(args: argparse.Namespace) -> int:
    ...
    observed = (
        None if args.result is None else json.loads(Path(args.result).read_text(encoding="utf-8"))
    )
    with ReplayLedger(args.ledger) as ledger:
...
def build_parser() -> argparse.ArgumentParser:
    ...
    verify_object = subparsers.add_parser(
        "verify-object", help="verify a recorded object against a schema"
    )
    ...
    replay_execution = subparsers.add_parser(
        "replay-execution", help="verify execution evidence and an optional observed result"
    )
    ...
    verify_environment = subparsers.add_parser(
        "verify-environment", help="compare the current environment with a recorded execution"
    )
    ...
    export_manifest = subparsers.add_parser(
        "export-manifest", help="export object and execution evidence"
    )
